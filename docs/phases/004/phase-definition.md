...
Refine concept boundaries so each retained concept serves a coherent purpose, contains the behavior necessary to fulfill that purpose, can be understood independently of other application concepts, and is generic where broader applicability improves the design.
...
Typically includes stabilized concept boundaries, specificity/completeness/independence findings, split/merge/reframe/reject decisions, genericity decisions, revised concept specifications, and remaining composition questions for Phase 005.
...
The phase begins with a mandatory `004-A` start gate, which derives project-specific modularity and boundary-review subphases. The final subphase is consolidation, exit review, and handoff.
